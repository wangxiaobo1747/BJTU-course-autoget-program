...
class CourseGrabber:
    ...
    async def get_cookie(
        ...
        try:
            ...
            # 获取选课页面并从中提取用户名
            course_page = await asyncio.to_thread(
                self.session.get,
                "https://aa.bjtu.edu.cn/course_selection/courseselecttask/selects/",
            )
            self._course_page_html = course_page.text
            self.username = await asyncio.to_thread(
                self._get_username, course_page.text
            )
    ...
    def _get_username(self, page_html: str) -> str:
        """从选课页面导航栏提取用户名"""
        soup = BeautifulSoup(page_html, "html.parser")
        # 方式1: 导航栏 <span class="user-info"><small>欢迎您，</small>王振洲</span>
        user_info = soup.find("span", class_="user-info")
        if user_info:
            small = user_info.find("small")
            if small and small.next_sibling:
                username = small.next_sibling.strip()
                if username:
                    return username
        # 方式2: 侧栏 <div id="sidebar-shortcuts-large">欢迎您，<strong>王振洲</strong></div>
        sidebar = soup.find(id="sidebar-shortcuts-large")
        if sidebar:
            strong = sidebar.find("strong")
            if strong:
                return strong.get_text(strip=True)
        raise Exception("无法从页面中提取用户名")
    ...
    def get_available_courses(self) -> List[Dict[str, str]]:
        ...
        try:
            if self.config.course_type == "required":
                # 优先使用登录时已缓存的选课页面
                if hasattr(self, '_course_page_html') and self._course_page_html:
                    html = self._course_page_html
                    self._course_page_html = None  # 仅首次使用缓存
                else:
                    url = (
                        "https://aa.bjtu.edu.cn/course_selection/courseselecttask/selects/"
                    )
            ...
            for row in tables[table_id].find_all("tr"):
                ...
                # 安全检查: 确保列数足够
                if len(cols) <= max(name_column, number_column, teacher_column):
                    continue
    ...
    def _check_course_valid(self, course: Dict[str, str]) -> bool:
        """检查课程是否符合选课条件（支持按课程名或教师名筛选）"""
        flag = False
        for key in self.config.course_list:
            if key in course["name"] or key in course["teacher"]:
                flag = True
